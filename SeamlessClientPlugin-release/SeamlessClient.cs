...
namespace SeamlessClientPlugin
{
    ...
    public class SeamlessClient : IPlugin
    {
        ...
        public void Init(object gameInstance) => TryShow($"Running Seamless Client Plugin v[{Version}]");
        ...
        public static void DisposeInit()
        {
        ...
        private static void MessageHandler(ushort obj1, byte[] obj2, ulong obj3, bool obj4)
        {
            try
            {
                var received = Utility.Deserialize<ClientMessage>(obj2);

                if (received.MessageType == ClientMessageType.FirstJoin)
                {
                ...
                else if (received.MessageType == ClientMessageType.TransferServer)
                {
                    //Server sent a transfer message! Begin transfer via seamless
                    var transferMessage = received.GetTransferData();
                    ServerPing.StartServerPing(transferMessage);
                }
                else if (received.MessageType == ClientMessageType.OnlinePlayers)
                {
                    var p = received.GetOnlinePlayers();
        ...
        public void Dispose() => DisposeInit();
    }
}